...
$servers | ForEach-Object -Parallel {
    ...
    if (!$session) {
        ...
        $result = Invoke-Command -Session $session -FilePath $($Using:cleanCommands)
        if ($result) {
            Add-content $($Using:successlog) -Value "$($server.servername);cleaned"
            Write-Host "Successfully cleaned host $($server.servername)"
        } 
        else {
            Add-content $($Using:successlog) -Value "$($server.servername);fail!!!"
            Write-Host "Error cleaning host $($server.servername)"
        }
    }
